...
### `internal/gh` as its own package, not a `git.Runner` extension (phase 15)
- **Decision:** `internal/gh` mirrors `internal/git`'s shape — a `Runner{Dir}`, free functions taking `(ctx, r, ...)`, a private parser (`parsePRs`) as the unit-testable seam — but is a standalone package, not a method set added onto `git.Runner`.
- **Why:** `git.Runner.Run` hardcodes `exec.Command("git", ...)`; there is no seam to redirect it at a different binary. A second exec target needs its own runner type. Mirroring the shape (rather than inventing a different one) keeps the two packages readable side by side and gives `gh` the same test story (parse fixture JSON, never invoke the real binary).
- **Rejected:** a generic `Runner` parameterized by binary name — one extra layer of indirection for a type used by exactly two packages; adding a `Binary` field to `git.Runner` — conflates two unrelated CLIs behind one type name.

### PR data is best-effort everywhere, never an error
- **Decision:** `gh.Available()` gates every call site; `gh.PRsByBranch` swallows any failure (no `gh`, no GitHub remote, not authenticated, rate-limited, anything) and returns `nil`, which behaves like an empty map for lookups. No flag or setting enables/disables the PR column — its presence follows `gh`'s availability alone.
- **Why:** PROMPT.md's shared model never blocks a view on an optional integration. A user without `gh` installed, or working in a non-GitHub repo, should see `branch`/`worktree` behave exactly as before phase 15 — an empty `pr` column, not a startup error or a warning to dismiss.
- **Rejected:** surfacing a status-line warning when `gh` is unavailable (noise on every invocation for most users); a `--no-pr`/settings toggle (nothing to toggle when failure is already silent and free).

### Async PR loading — the first background `tea.Cmd` (phase 15)
- **Decision:** `branch` and `worktree`'s interactive views fetch PRs via `Config.InitCmd`, a `tea.Cmd` returned from `List.Init()` (batched with the existing `textinput.Blink` direct-menu path) that runs the `gh pr list` subprocess on its own goroutine and feeds the result back as an ordinary `itemsMsg` once it lands. `-I`/`RenderTable` output has no TUI to fill in later, so it fetches synchronously instead — a one-shot print can afford the round trip.
- **Why:** a `gh` call is a subprocess spawn (tens to hundreds of ms, more over a slow network for `gh`'s own API round trip) — blocking `Init()` on it would freeze the list's first paint for a feature most invocations don't even look at. Every other list-mutating code path in the codebase runs synchronously inside `Update` (see "resilient bulk delete" above); this is the first one that genuinely needs to run off the update goroutine, which is why the sort/grouping/PR-cache fields moved behind a `sync.Mutex` with copy-on-write map replacement (`toggleCollapsed`, `setPRCache`) instead of the direct field access every other operation still uses — plain field mutation on the update goroutine, read without synchronization by a concurrent background goroutine, is exactly the data race `-race` exists to catch.
- **Rejected:** fetching PRs synchronously before the interactive view opens (reintroduces the blocking-`Init()` problem this decision exists to avoid); a generic "background loader" abstraction in `internal/tui` (one call site pattern doesn't earn a framework layer yet — revisit if a third view needs the same shape).

### Checkout-into-another-worktree prompt via `Operation.ConfirmFrom`
- **Decision:** `branch`'s checkout operation gains `Operation.ConfirmFrom func([]Item) *Confirm`, a new field on `Operation` that resolves the confirmation from the target row at invocation time (returning `nil` runs with no prompt), overriding a static `Confirm`. It fires only when the target branch's `wtPath` is non-empty and it isn't this worktree's `HEAD`, offering "no" / "cd there"; choosing "cd there" quits the view and hands the path back to the shell exactly like `worktree`'s own checkout (needs the same `shell-init` wrapper, now matching `branch`/`br` too, not just `worktree`/`wt`).
- **Why:** `git checkout` on a branch already checked out elsewhere fails with git's raw `fatal: 'x' is already used by worktree at '...'`, dumped verbatim into the one-line footer — unhelpful, and every row already carries the owning worktree's path (`branchItem.wtPath`), so gint can offer the obvious next step instead of just erroring. A static `Operation.Confirm` can't express "only prompt for some rows," since `List.runConfirm` resolves it once per operation, not per invocation — hence the new field rather than a per-row hack.
- **Rejected:** always confirming checkout with an extra step (the common case — no worktree conflict — should stay a single keystroke); folding "remove the worktree" into this prompt (narrowed away during review — removal stays exactly where it already lives, `worktree`'s own delete operation).

### `Searchable` interface widens fuzzy search without changing `FilterValue` (phase 15)
- **Decision:** an optional `tui.Searchable` interface (`SearchValue() string`) lets an item expose extra fuzzy-searchable text; `filterItems` prefers it when present, falling back to `FilterValue()`. `worktreeItem`/`branchItem` implement it (raw + formatted worktree path, branch name, PR number) while `FilterValue()` stays exactly what it was.
- **Why:** the search audit found one real hole — `worktreeItem.FilterValue()` returns the raw absolute path, but the column renders it through `FormatWorktreePath`, so under the default `shortest` format a query typed against the `~`-abbreviated path visible on screen matched nothing. `FilterValue` can't simply be widened to the formatted/multi-field form because it doubles as the human-readable label in resilient bulk operations (`batch.go`'s failure summaries) — widening it would start reporting `feature/x ~/src/wt #412 open` instead of the branch name on a failed bulk delete.
- **Rejected:** widening `FilterValue` directly (breaks batch-failure labels, see above); a separate `SearchLabel` map maintained alongside items (redundant with data the item already holds).
